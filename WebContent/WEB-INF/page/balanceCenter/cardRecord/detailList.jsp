...
<div class="pageHeader" style="border: 1px #B8D0D6 solid">
	<form action="${base}/detailList.do" id="pagerForm">
		...
		<div class="searchBar">
			<table border="0">
				<tr align="right">
					<td width="60">查询类型：</td>
					<td align="left">
						<select class="combox" outerw="80" innerw="100" name="queryType">
							<option value="0" width="100" <c:if test="${queryType==0}">selected="selected"</c:if>>统计查询</option>
							<option value="1" width="100" <c:if test="${queryType==1}">selected="selected"</c:if>>明细查询</option>
						</select>
					</td>
					<td width="60">出纳员：</td>
					<td align="left">
				...
				<tr>
					...
					<td align="left" colspan="2">
						<select class="combox" outerw="80" innerw="100" name="recordType">
							<option value="-1" width="100" <c:if test="${status.index==-1}">selected</c:if>>全部</option>
							<c:forEach var="r" items="${recordTypes}" varStatus="status">
								<option value="${status.index}" width="100" <c:if test="${status.index==recordType}">selected</c:if>>${r}</option>
							</c:forEach>
						</select>
					</td>		
					<td colspan="12">
